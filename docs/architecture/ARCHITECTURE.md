...
> **Companion docs:** [API-INTEGRATION.md](./API-INTEGRATION.md) · [STATE-MANAGEMENT.md](./STATE-MANAGEMENT.md) · [ERROR-HANDLING.md](./ERROR-HANDLING.md) · [PERFORMANCE-AND-CACHING.md](./PERFORMANCE-AND-CACHING.md) · [TESTING-STRATEGY.md](./TESTING-STRATEGY.md) · [ACCESSIBILITY.md](./ACCESSIBILITY.md) · [CODING-GUIDELINES.md](../CODING-GUIDELINES.md) · [DESIGN-SYSTEM.md](../design/DESIGN-SYSTEM.md) · [PRODUCTION-READINESS.md](../PRODUCTION-READINESS.md) · [ADRs](../adr)

---

## 1. The problem, restated as constraints

The functional requirements (garden + plant CRUD, target humidity, overcrowding validation) are ordinary. What makes this case interesting is what the backend _does to us on purpose_:
...
**North star:** the reviewer should never _feel_ the slow, flaky backend while using the app — yet the code should make it obvious we know exactly how hostile it is.
...
| UI kit    | **Angular Material 22** (M3) + custom design tokens                      | Accessible primitives for free (dialogs, menus, forms, a11y), themed heavily so it doesn't look like stock Material. [DESIGN-SYSTEM.md](../design/DESIGN-SYSTEM.md)                                          |
...
│   ├── garden-detail/         # single garden: Garden Map (digital twin), plants table
...
| **Functional**          | overcrowding 400, validation 400 | Never retried. Rendered _in context_: inline on the field or as a form-level message. These are expected behaviour, logged at `info`.                               |
...
## 8. What we deliberately did _not_ build
