...
Zoneless + OnPush + signals (change detection only where a signal changed); every route is a lazy `loadComponent` chunk; the Garden Map ships in its own `@defer (on viewport; prefetch on idle)` chunk behind a dimension-matched ghost — measured, its value here is the **lazy chunk boundary**, not delayed work: on a desktop viewport the map is above the fold, so the trigger fires immediately (the trigger is self-tuning — it genuinely defers only on viewports where the map starts off-screen), and the dimension-matched placeholder keeps CLS at 0.025 on a 1440×900 laptop and 0.000 on a 375×812 phone; stable `track` on every `@for`; capacity bars animate on `transform` only; self-hosted variable fonts with `font-display: swap`, the two latin faces preloaded from `index.html` (media files keep their names, so the URL is stable); icons inline. The app shell imports no Material: the account menu — Material's menu and the CDK overlay behind it, 129 kB of the initial bundle when the shell imported it — is a `@defer (on idle)` block whose placeholder is the same chip, so nothing moves when it arrives. Production initial: 381 kB raw, ~107 kB transferred (from 511 kB / 138 kB), enforced by `angular.json` budgets — the initial bundle and each named chunk (dashboard, garden-list, garden-detail, garden-map, onboarding) — so the build fails on regression; and `npm run bundle:check` diffs every chunk of a build against the committed `apps/web/bundle-baseline.json` in CI (a chunk may grow 5 kB or 3 % for the initial bundle, 10 % for a named chunk, before it fails), so growth is a reviewed decision, not a surprise. The two Highcharts charts follow the same rule: the library (about 127 kB transferred) is never in a route chunk; the two screens with a chart fetch it while the browser is idle (ADR-008).
