...
## Context
The functional requirements demand *"Each garden should have a configurable target humidity level (range: 0-100)"*, but the provided API has no such field on Garden (verified in `garden.schema.ts`, `migration001.ts`, `database/types.ts`). Plants have `idealHumidityLevel`; gardens do not have a target. The case explicitly permits API changes ("you are free to make those changes").
