...
describe('CapacityBar', () => {
  const render = (used: number, total = 20) => {
    ...
    return { fixture, el, track: el.querySelector('.capacity-bar__track')! };
  };

  it('shows a summed area to two decimals, never raw float noise', () => {
    const { el, track } = render(1.2 + 3 + 0.6); // 4.800000000000001
    expect(el.querySelector('.capacity-bar__label span')?.textContent?.trim()).toBe('4.8 / 20 m²');
  ...
  it('keeps real decimals up to two places', () => {
    ...
    expect(el.querySelector('.capacity-bar__label span')?.textContent?.trim()).toBe(
      '13.35 / 24.5 m²',
    );
  });
  ...
  it.each([
    ...
    expect(el.querySelector('.capacity-bar__fill')?.classList).toContain(
      `capacity-bar__fill--${level}`,
    );
  });

  it('draws the fill as a clamped transform, and can hide its caption', () => {
    const { fixture, el } = render(30); // over capacity
    expect((el.querySelector('.capacity-bar__fill') as HTMLElement).style.transform).toBe(
      'scaleX(1)',
    );
    expect(el.querySelector('.capacity-bar__pct')?.textContent?.trim()).toBe('150%');

    fixture.componentInstance.showLabel.set(false);
    fixture.detectChanges();
    expect(el.querySelector('.capacity-bar__label')).toBeNull();
  });
});
